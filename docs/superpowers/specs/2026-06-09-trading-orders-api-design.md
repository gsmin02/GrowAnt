...
- `backend/src/main/resources/application.yml` 최상단에 추가(의존성 없음, Spring 내장):
  ...
  기존 `${DB_URL}` 등 placeholder(자체 호스팅 PostgreSQL)가 이 골격과 연결되며, 지금은 local 프로파일이 DB/Redis 자동설정을 제외하므로 no-op.
- 루트 `.env.example`은 이미 포괄적(PostgreSQL·Redis·Gemini·JWT·OAuth·MARKET_PROVIDER) — 무변경.
